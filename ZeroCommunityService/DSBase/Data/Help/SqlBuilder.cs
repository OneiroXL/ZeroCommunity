...
namespace DSBase.Data.Help
{
    public static class SqlBuilder
    {
        public class SelectBuilder
        {
            ...
            public SelectBuilder WhereOr(string sql, dynamic parameters = null)
            {
                AddClause("where", sql, parameters, " or ", prefix: "WHERE ", postfix: "\n");
                return this;
            }

            public SelectBuilder WhereOr(bool condition, string sql, dynamic parameters = null)
            {
                if (condition)
                {
                    AddClause("where", sql, parameters, " or ", prefix: "WHERE ", postfix: "\n");
            ...
            public CommandDefinition ToCommand(int take)
            {
                ...
                {
                    sb.Append("SELECT TOP (@RowTake) ");
                    param.Add("@RowTake", take);
                }
            ...
            public CommandDefinition ToCommand(int page, int size)
            {
                ...
                if (!data.ContainsKey("orderby"))
                {
                    throw new ArgumentException("order by 参数未提供");
                }

                var start = (page - 1) * size + 1;
                var end = page * size;

                param.Add("@RowBegin", start);
                param.Add("@RowEnd", end);
                ...
                //SELECT * FROM (SELECT ROW_NUMBER() OVER (/**orderby**/) AS Row,/**field**/ FROM /**table**/ /**where**/) AS T WHERE Row BETWEEN @Begin AND @End ;
                ...
                if (data.ContainsKey("having"))
                    sb.Append(data["having"]);
                sb.Append(") AS [Temp.Table] WHERE [Temp.Table].[RowNumber] BETWEEN @RowBegin AND @RowEnd ");
                if (data.ContainsKey("orderby"))
                {
                    sb.Append(data["orderby"]);
                }
            ...
            public CommandDefinition ToCommand(int page, int size, string childOrder)
            {
                if (page < 1)
                {
                    page = 1;
                ...
                if (!data.ContainsKey("orderby"))
                {
                    throw new ArgumentException("roder by 参数未提供");
                }
                ...
                //SELECT * FROM (SELECT ROW_NUMBER() OVER (/**orderby**/) AS Row,/**field**/ FROM /**table**/ /**where**/) AS T WHERE Row BETWEEN @Begin AND @End ;
                sb.Append("SELECT * FROM (SELECT ROW_NUMBER() OVER (");
                if (!string.IsNullOrEmpty(childOrder))
                {
                    sb.Append("ORDER BY " + childOrder + " ");
                }
                else
                {
                    if (data.ContainsKey("orderby"))
        ...
        public class InsertBuilder
        {
            ...
            public CommandDefinition ToCommand(bool identity = false, IDbTransaction transaction = null, int? timeout = null)
            {
                ...
                var panmes = string.Join(",", columns.Keys.Select(o => "@" + o.TrimStart('[').TrimEnd(']')));
                ...
                var param = new DynamicParameters(columns.ToDictionary(x => x.Key.TrimStart('[').TrimEnd(']'), x => x.Value));
        ...
        public class UpdateBuilder
        {
            ...
            public CommandDefinition ToCommand(IDbTransaction transaction = null, int? timeout = null)
            {
                ...
                {
                    sb.AppendFormat(i == 0 && statics.Count == 0 ? "{0}=@{1}\n" : "    ,{0}=@{1}\n", o.Key, o.Key.TrimStart('[').TrimEnd(']'));
                    i++;
                }
                var param = new DynamicParameters(columns);

                // if (wheres.Count <= 0) return new CommandDefinition(sb.ToString(), param);
