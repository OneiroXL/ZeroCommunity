...
namespace Base.PublicTools.Base
{
    public class RedisBaseTool
    {
        /// <summary>
        /// 连接字符串
        /// </summary>
        ...
        static RedisBaseTool()
        {
            ConnectionString = "192.168.199.145:6000";
            ConnMultiplexer = ConnectionMultiplexer.Connect(ConnectionString);
            AddRegisterEvent();
        }

        public RedisBaseTool(int db = -1)
        {
        ...
        {
            var points = ConnMultiplexer.GetEndPoints();
            List<string> keyList = new List<string>();
            foreach (var ep in points)
            {
                var server = ConnMultiplexer.GetServer(ep);
                var keys = server.Keys(1, "*");
                foreach (var item in keys)
                {
                    if (((string)item).IndexOf(keyPreFix) > -1)
                    {
